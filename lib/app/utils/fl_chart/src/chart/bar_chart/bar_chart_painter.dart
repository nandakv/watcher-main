...
import '../../../../fl_chart/src/extensions/path_extension.dart';

import '../../../../fl_chart/src/extensions/bar_chart_data_extension.dart';

import '../../../../fl_chart/src/extensions/paint_extension.dart';
import '../../../../fl_chart/src/extensions/rrect_extension.dart';
...
class BarChartPainter extends AxisChartPainter<BarChartData> {
  ...
  void drawTouchTooltip(
    ...
    Get.log("barToYPixel - $barToYPixel");
    Get.log("barFromYPixel - $barFromYPixel");
    ...
    Get.log("barTopY - $barTopY");
    Get.log("barBottomY - $barBottomY");
    ...
    Get.log("drawTooltipOnTop - $drawTooltipOnTop");

    final tooltipTop = drawTooltipOnTop
        ? barTopY - tooltipHeight - tooltipData.tooltipMargin
        : barBottomY + tooltipData.tooltipMargin;

    Get.log("tooltipTop - $tooltipTop");

    Get.log("tooltip width - $tooltipWidth");

    Get.log("viewSize.width - ${viewSize.width}");

    Get.log("barFromYPixel.dx - ${barFromYPixel.dx}");

    double tooltipLeft;

    bool isToolTipOverflow = (barFromYPixel.dx + tooltipWidth) > viewSize.width;

    if (isToolTipOverflow) {
    ...
    final shadowPaint = Paint()
      ..color = const Color(0xff5BC4F0) // Set shadow color
      ..maskFilter =
          const MaskFilter.blur(BlurStyle.normal, 1); // Set shadow blur

    // Shadow for the rounded rectangle
    ...
    // Apply shadow for the rect
    canvasWrapper.canvas.drawRRect(roundedRect, shadowPaint);
    ...
    // Draw the arrow with shadow
    const arrowHeight = 14.0; // Height of the arrow
    const arrowWidth = 10.0; // Width of the arrow
    const double cornerRadius = 2.0;

    final arrowPath = Path();

    if (!isToolTipOverflow) {
      final rectLeft = rect.left + 1;
      ...
      arrowPath
        ..moveTo(
            ...
            rect.top +
                rect.height / 2 -
                arrowHeight / 2) // Start at top of arrow
        ..lineTo(
            rectLeft - arrowWidth + cornerRadius,
            rect.top +
                rect.height / 2 -
                cornerRadius) // Line to start of the rounded tip
        ..arcToPoint(
          Offset(rectLeft - arrowWidth + cornerRadius,
              rect.top + rect.height / 2 + cornerRadius),
          radius: const Radius.circular(cornerRadius),
          clockwise: false,
        ) // Rounded tip
        ..lineTo(rectLeft,
            rect.top + rect.height / 2 + arrowHeight / 2) // Bottom of the arrow
        ..close();
    } else {
      final rectRight = rect.right - 1; // Adjust right edge of the rect
      ...
      arrowPath
        ..moveTo(
            ...
            rect.top +
                ...
                arrowHeight / 2) // Start at top of arrow
        ..lineTo(
            rectRight + arrowWidth - cornerRadius,
            rect.top +
                rect.height / 2 -
                cornerRadius) // Line to start of the rounded tip
        ..arcToPoint(
          Offset(rectRight + arrowWidth - cornerRadius,
              rect.top + rect.height / 2 + cornerRadius),
          radius: const Radius.circular(cornerRadius),
          clockwise: true,
        ) // Rounded tip
    ...
    canvasWrapper.canvas.drawPath(arrowPath, shadowPaint);
    canvasWrapper.canvas.drawPath(arrowPath, _bgTouchTooltipPaint);

    // Draw rotated tooltip with text
