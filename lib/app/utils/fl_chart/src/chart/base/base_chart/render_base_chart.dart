...
abstract class RenderBaseChart<R extends BaseTouchResponse> extends RenderBox
  ...
  _startTimer() {
    _timer = Timer(const Duration(seconds: 3), () {
      _notifyTouchEvent(const FlTapCancelEvent(), "timer", onCancel: true);
    });
    Get.log("timer started");
  }

  /// Initializes our recognizers and implement their callbacks.
  void initGestureRecognizers() {
    Get.log("initGestureRecognizers");
    _panGestureRecognizer = PanGestureRecognizer();
    _panGestureRecognizer
      ..onDown = (dragDownDetails) {
        Get.log("_tapGestureRecognizer - onPanDown");
        _notifyTouchEvent(FlPanDownEvent(dragDownDetails), "onPanDown");
      }
      ..onStart = (dragStartDetails) {
        Get.log("_tapGestureRecognizer - onPanStart");
        _notifyTouchEvent(FlPanStartEvent(dragStartDetails), "onPanStart");
      }
      ..onUpdate = (dragUpdateDetails) {
        Get.log("_tapGestureRecognizer - onPanUpdate");
        _notifyTouchEvent(FlPanUpdateEvent(dragUpdateDetails), "onPanUpdate");
      }
      ..onCancel = () {
        Get.log("_tapGestureRecognizer - onPanCancel");
        _startTimer();
      }
      ..onEnd = (dragEndDetails) {
        Get.log("_tapGestureRecognizer - onPanEnd");
        _startTimer();
      };
    ...
    _tapGestureRecognizer
      ..onTapDown = (tapDownDetails) {
        Get.log("_tapGestureRecognizer - onTapDown");
        _notifyTouchEvent(FlTapDownEvent(tapDownDetails), "onTapDown");
        if (_timer != null && _timer!.isActive) {
          _timer!.cancel();
        }
      }
      ..onTapCancel = () async {
        Get.log("_tapGestureRecognizer - onTapCancel");
        _startTimer();
      }
      ..onTapUp = (tapUpDetails) async {
        Get.log("_tapGestureRecognizer - onTapUp");
        _startTimer();
      };
    ...
    _longPressGestureRecognizer
      ..onLongPressStart = (longPressStartDetails) {
        Get.log("_tapGestureRecognizer - onLongPressStart");
        if (_timer != null && _timer!.isActive) _timer!.cancel();
        _notifyTouchEvent(
            FlLongPressStart(longPressStartDetails), "onLongPressStart");
      }
      ..onLongPressMoveUpdate = (longPressMoveUpdateDetails) {
        Get.log("_tapGestureRecognizer - onLongPressMoveUpdate");
        _notifyTouchEvent(FlLongPressMoveUpdate(longPressMoveUpdateDetails),
            "onLongPressMoveUpdate");
      }
      ..onLongPressEnd = (longPressEndDetails) async {
        Get.log("_tapGestureRecognizer - onLongPressEnd");
        _startTimer();
      };
  }
  ...
  void handleEvent(PointerEvent event, covariant BoxHitTestEntry entry) {
    ...
    if (event is PointerDownEvent) {
      ...
      _notifyTouchEvent(FlPointerHoverEvent(event), "PointerHoverEvent");
    }
  }

  /// Here we handle mouse hover enter event
  @override
  PointerEnterEventListener? get onEnter => (event) => _notifyTouchEvent(
      FlPointerEnterEvent(event), "PointerEnterEventListener");

  /// Here we handle mouse hover exit event
  @override
  PointerExitEventListener? get onExit => (event) =>
      _notifyTouchEvent(FlPointerExitEvent(event), "PointerExitEventListener");

  R? _previousResponse;
  FlTouchEvent? _previousTouchEvent;
  ...
  void _notifyTouchEvent(FlTouchEvent event, String function,
      {bool? onCancel}) {
    if (_touchCallback == null) {
      Get.log("_touchCallback = null");
      return;
    }
    Get.log("_touchCallback != null, $function");
    ...
    if (localPosition != null) {
      ...
      if(response is LineTouchResponse){
        Get.log("response is LineTouchResponse");
        LineTouchResponse lineTouchResponse = response as LineTouchResponse;
        if(lineTouchResponse.lineBarSpots == null){
          Get.log("lineTouchResponse.lineBarSpots is null");
          return;
        }
      }
      if(response is BarTouchResponse){
        Get.log("response is BarTouchResponse");
        BarTouchResponse barTouchResponse = response as BarTouchResponse;
        if(barTouchResponse.spot == null){
          Get.log("barTouchResponse.spot is null");
          return;
        }
      }
    }

    if (onCancel != null && onCancel) response = null;
    _previousResponse = response;
    _previousTouchEvent = event;
