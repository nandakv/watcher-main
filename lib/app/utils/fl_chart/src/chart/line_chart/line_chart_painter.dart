import 'dart:math';
import '../../../../fl_chart/src/chart/base/axis_chart/axis_chart_extensions.dart';
...
class LineChartPainter extends AxisChartPainter<LineChartData> {
  ...
  void drawTouchTooltip(
    ...
    if (tooltipItems.length != showingTooltipSpots.showingSpots.length) {
      throw Exception('tooltipItems and touchedSpots size should be the same');
    }
    ...
    _bgTouchTooltipPaint.color =
        tooltipData.getTooltipColor(showingTooltipSpots.showingSpots.first);

    /// Draw tooltip rectangle
    canvasWrapper.canvas.drawRRect(roundedRect, _bgTouchTooltipPaint);
    ...
    final arrowPath = Path()
      ..moveTo(
          rect.center.dx - arrowWidth / 2, rect.bottom - 1) // Bottom left of arrow
      ..lineTo(rect.center.dx + arrowWidth / 2, rect.bottom - 1) // Bottom right
      ..lineTo(rect.center.dx, rect.bottom + arrowHeight) // Tip of the arrow
      ..close();
    ...
    for (final tp in drawingTextPainters) {
      ...
      tp.paint(canvasWrapper.canvas, drawOffset);
      topPosSeek += tp.height;
      topPosSeek += textsBelowMargin;
    }
  }

  dynamic _computeXOffset(HorizontalAlignment align, Rect rect,
      LineTouchTooltipData tooltipData, TextPainter tp) {
...
class TooltipPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Tooltip body with rounded corners
    final bodyRect = Rect.fromLTRB(0, 0, size.width, size.height - 10);
    final bodyRRect = RRect.fromRectAndRadius(bodyRect, Radius.circular(12));
    canvas.drawRRect(bodyRRect, paint);

    // Tooltip tip (triangle)
    final tipPath = Path();
    tipPath.moveTo(size.width / 2 - 10, size.height - 10); // Left point of tip
    tipPath.lineTo(size.width / 2 + 10, size.height - 10); // Right point of tip
    tipPath.lineTo(size.width / 2, size.height); // Bottom point of tip
    tipPath.close();
