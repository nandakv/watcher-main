...
class AxisOffset{
  late MinMax xAxis;
  late MinMax yAxis;
  AxisOffset({required this.xAxis, required this.yAxis});
}
...
class _LineChartState extends AnimatedWidgetBaseState<LineChart> {
  ...
  LineChartData _getData() {
    ...
    if (newData.minX.isNaN ||
        ...
        newData.minY.isNaN ||
        newData.maxY.isNaN) {
      final AxisOffset axisOffset = _lineChartHelper.calculateMaxAxisValues(
        newData.lineBarsData,
      );
      newData = newData.copyWith(
        minX: newData.minX.isNaN ? axisOffset.xAxis.min : newData.minX,
        maxX: newData.maxX.isNaN ? axisOffset.xAxis.max : newData.maxX,
        minY: newData.minY.isNaN ? axisOffset.yAxis.min : newData.minY,
        maxY: newData.maxY.isNaN ? axisOffset.yAxis.max : newData.maxY,
      );
    }
